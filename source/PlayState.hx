...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['Bay acc', 0.2], // From 0% to 19%
		['Ngu', 0.4], // From 20% to 39%
		['U Suck', 0.5], // From 40% to 49%
		['Bruh', 0.6], // From 50% to 59%
		['Eh', 0.69], // From 60% to 68%
		['Noice', 0.7], // 69%
		['Gud Boi', 0.8], // From 70% to 79%
		['great job', 0.9], // From 80% to 89%
		['Omg!', 1], // From 90% to 99%
		['GAMER!!', 1] // The value on this one isn't used actually, since Perfect is always "1"
	];
	...
	override public function create()
	{
		...
		if (dialogueJson == null && FileSystem.exists(file))
		{ // Only check if dialogue has not already been found
			dialogueJson = DialogueBoxPsych.parseDialogue(file);
		}
		#end
		...
		timeTxt.setFormat(Paths.font("font gamer.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		scoreTxt.setFormat(Paths.font("font gamer.ttf"), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		scoreTxt.scrollFactor.set();
		scoreTxt.borderSize = 1.25;
		scoreTxt.visible = !ClientPrefs.hideHud;
		add(scoreTxt);

		botplayTxt = new FlxText(400, timeBarBG.y + 55, FlxG.width - 800, "GAMER MODE", 32);
		if (ClientPrefs.downScroll)
		...
		botplayTxt.setFormat(Paths.font("font gamer.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	override public function update(elapsed:Float)
	{
		...
		if (ratingName == '?')
		{
			scoreTxt.text = 'Score: ' + songScore + ' | Dead Meme: ' + songMisses + ' | Accuracy: ' + ratingName;
		}
		else
		{
			scoreTxt.text = 'Score: ' + songScore + ' | Dead Meme: ' + songMisses + ' | Accuracy: ' + ratingName + ' ('
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			if (!note.isSustainNote && ClientPrefs.hitSounds)
