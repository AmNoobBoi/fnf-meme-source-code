...
class GameplaySettingsSubState extends BaseOptionsMenu
{
	public function new()
	{
		...
		var option:Option = new Option('Instant Respawn', "when u ded u will be respawned automatically, without animation(MisterRafers)", 'instantRespawn',
			'bool', false);
		addOption(option);

		var option:Option = new Option('Middlescroll', 'ur notes get centered.', 'middleScroll', 'bool', false);
		addOption(option);

		var option:Option = new Option('Ghost Tapping', "u won't get misses from pressing keys\nwhile there are no notes able to be hit.", 'ghostTapping',
			'bool', true);
		addOption(option);

		var option:Option = new Option('Hitsounds', 'when u hit notes it have sound gamer (Sector03)', 'hitSounds', 'bool', false);
		addOption(option);

		var option:Option = new Option('Disable Reset Button', "pressing Reset won't do anything.", 'noReset', 'bool', false);
		addOption(option);
