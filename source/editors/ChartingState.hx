	...
	**/
	...
	override function create()
	{
		...
		for (i in 0...tipTextArray.length)
		{
			...
			tipText.y += i * 10;
			tipText.setFormat(Paths.font("font gamer.ttf"), 10, FlxColor.WHITE, LEFT /*, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK*/);
		...
		if (FlxG.mouse.pressed && !FlxG.keys.pressed.TAB || FlxG.mouse.justPressed || FlxG.keys.pressed.C)
		{
			...
			else
			{
				if (FlxG.mouse.x > gridBG.x
					...
					&& FlxG.mouse.y < gridBG.y + (GRID_SIZE * _song.notes[curSection].lengthInSteps) * zoomList[curZoom] && mouseAction != 2)
				{
