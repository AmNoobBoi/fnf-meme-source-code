...
class MainMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		bg.setGraphicSize(Std.int(bg.width * 1.075));
		bg.updateHitbox();
		bg.screenCenter();
		...
		magenta = new FlxSprite(-40).loadGraphic(Paths.image('menuDesat'));
		magenta.scrollFactor.set(0, yScroll);
		magenta.setGraphicSize(Std.int(magenta.width * 1.075));
		...
		var menuItem:FlxSprite = new FlxSprite(80, 120);
		...
		// FreePlay Mode
		var offset:Float = 108 - (Math.max(optionShit.length, 4) - 4) * 80;
		var menuItem:FlxSprite = new FlxSprite(80, 250);
		menuItem.scale.x = scale;
		menuItem.scale.y = scale;
		...
		// Credits
		var offset:Float = 108 - (Math.max(optionShit.length, 4) - 4) * 80;
		var menuItem:FlxSprite = new FlxSprite(80, 400);
		menuItem.scale.x = scale;
		menuItem.scale.y = scale;
		...
		// Options
		var offset:Float = 108 - (Math.max(optionShit.length, 4) - 4) * 80;
		var menuItem:FlxSprite = new FlxSprite(80, 550);
		menuItem.scale.x = scale;
		menuItem.scale.y = scale;
		...
		var versionShit:FlxText = new FlxText(12, FlxG.height - 44, 0, "Psych Engine Source Code" + psychEngineVersion, 12);
		versionShit.scrollFactor.set();
		versionShit.antialiasing = true;
		versionShit.setFormat("FONT GAMER OSD Mono", 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		add(versionShit);
